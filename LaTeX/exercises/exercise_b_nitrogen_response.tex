...
% Learning Goals: N response curves; NUE calculation; cost-benefit analysis
...
\textbf{Deliverable:} Excel workbook with N response curve, NUE calculations, cost-benefit analysis.
...
\toprule
\textbf{N Rate (kg/ha)} & \textbf{Yield (kg/ha)} & \textbf{NUE (kg/kg)} \\
\midrule
0 & 2000 & -- \\
...
\section{Task 3: Calculate Nitrogen Use Efficiency (NUE)}

\[
\text{NUE}_{i} = \frac{\text{Yield}_{i} - \text{Yield}_{0 \text{ kg/ha}}}{N \text{ rate}_{i}} \quad [\text{kg grain / kg N}]
\]
...
\begin{itemize}
  ...
  \item NUE(80) = (3500 -- 2000) / 80 = 18.75 kg grain / kg N
\end{itemize}

\textbf{Observations:}
\begin{itemize}
  \item NUE is highest at low N rates (diminishing returns)
  \item NUE decreases as N rate increases (approaching physiological limit)
  \item NUE reflects N use efficiency; typical wheat: 10--20 kg/kg
\end{itemize}
...
\begin{enumerate}
  ...
  \item At what N rate does NUE drop to 10 kg/kg? (Where do diminishing returns become pronounced?)
...
\begin{itemize}
  ...
  \item $\checkmark$ NUE calculations (table with NUE for each N rate)
