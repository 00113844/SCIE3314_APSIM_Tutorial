...
\begin{forest}
  for tree={
    ...
    rounded corners=3pt,
    font=\small\ttfamily,
    edge={draw=UWAGrey!60, -stealth, thin},
    l=10mm, s sep=1.5mm,
    minimum height=6.5mm,
    minimum width=2.2cm,
    align=center,
    inner ysep=3pt, inner xsep=4pt,
  }
  [Simulations, fill=UWABlue!22, font=\small\bfseries\ttfamily
    [Simulation, fill=UWABlue!15
      [Clock]
      [Weather]
      [{Soil\\Arbitrator}, align=center]
      [{Micro\\Climate}, align=center]
      [{Zone (Field)}, fill=UWABlue!15
        [Report]
        [Fertiliser]
        [Soil, fill=UWABlue!15
          [Physical]
          [SoilWater]
          [Organic]
          [{Nutrient\\NO3{,}NH4}, align=center]
          [Water]
        ]
        [{Surface\\OrgMatter}, align=center]
        [Wheat]
        [{Sow using\\var. rule}, align=center]
        [{Fertilise\\at sowing}, align=center]
        [Harvest]
      ]
      [{Graph\\$>$Series}, align=center]
    ]
    [DataStore]
  ]
\end{forest}%
}
\caption{Component hierarchy of a typical APSIM wheat simulation. Each indented node is a child of the one above it --- mirroring what you see in the APSIM left panel.}
\label{fig:module1-tree}
\end{figure}
...
\begin{lstlisting}[caption=Simplified .apsimx structure]
{
  ...
  "Children": [
    {
      ...
      "Children": [
        ...
        },
        {
          ...
          "FileName": "%root%/Examples/WeatherFiles/AU_Northam.met",
          "Name": "Weather"
        },
        { ... more components ... }
      ]
...
\toprule
\textbf{Component} & \textbf{Type} & \textbf{Configuration} \\
\midrule
Clock & Input & Dates (start, end), timestep \\
Weather & Input & File path (\filename{.met}), variables \\
Soil & State & Layers, properties, initial water/N \\
Wheat & Process & Phenology, LAI, grain fill \\
Manager & Control & Rules (JavaScript or C\#) \\
Report & Output & Variable list \\
DataStore & Output & Database (auto-populated) \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{description}
  \item[\textbf{Location}] Northam, Western Australia (Red/Brown earth soil)
...
\begin{tipbox}
\textbf{Note:} Detailed component reference pages are planned for the APSIM tutorial online knowledge base. For now, consult the official APSIM Next Gen documentation at:\\[0.3em]
\url{https://apsimnextgeneration.netlify.app/} \\[0.3em]
Key topics to explore: Clock (simulation timing), Weather (climate input), Soil (profile configuration), Wheat (crop model), Manager (management rules), Report (output variables), and DataStore (results database).
\end{tipbox}
