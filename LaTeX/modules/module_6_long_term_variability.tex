...
\subsection{Inter-Annual Variability in the Western Australian Wheatbelt}

Northam (Western Australian Wheatbelt) experiences high year-to-year rainfall variability:

\begin{itemize}
  \item \textbf{Annual rainfall range}: Roughly 300--450 mm/year (long-term mean $\approx$ 370 mm)
  \item \textbf{Coefficient of Variation (CV) of rainfall}: 30--40\% --- meaning a ``typical'' year can be 100--150 mm above or below average
  \item \textbf{ENSO influence}: El Ni\~{n}o years tend to be drier; La Ni\~{n}a years wetter
  \item \textbf{In-season variability}: Drought can occur from October onwards, particularly during critical grain fill period (October--November)
\end{itemize}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{0.85\textwidth}{\centering\vspace{1.5cm}
    \textit{[IMAGE: Bar chart of annual rainfall at Northam, 1950--1969 from the weather file.}\\
    \textit{Annotate: long-term mean as horizontal dashed line; highlight 3 driest and 3 wettest years.]}
  \vspace{1.5cm}}}
  \caption{Annual rainfall variability at Northam (WA Wheatbelt), 1950--1969 (from Exercise D weather data). Note the high year-to-year variation and lower overall rainfall compared to eastern Australia.}
...
\subsection{The APSIM Experiment Node}

APSIM's \textbf{Experiment} node allows you to run multiple treatment combinations automatically. Inside an Experiment, a \textbf{Factors} node contains one or more \textbf{Factor} nodes that specify which parameter to vary and what values to test.

The Factor specification syntax is:
\begin{verbatim}
[ComponentName].Script.ParameterName = value1, value2, value3
\end{verbatim}
or as a range:
\begin{verbatim}
[ComponentName].Script.ParameterName = start to end step increment
\end{verbatim}

APSIM automatically generates a separate simulation for each combination. For multi-factor experiments, all combinations are created (e.g., 3 sowing dates $\times$ 4 N rates = 12 simulations).
...
Expected outcomes:
\begin{itemize}
  \item Wet years: Good autumn/winter rains (May--September) $\to$ high ESW at sowing $\to$ high yield
  \item Dry years: Poor in-season rains or hot spring winds $\to$ water stress during grain fill (October--November) $\to$ low yield
  \item Yield CV: 30--50\% is typical for rainfed wheat in the Mediterranean WA Wheatbelt
\end{itemize}
...
\begin{enumerate}
  \item Northern WA Wheatbelt rainfall CV is 30--40\%; single-season analysis is insufficient for reliable management recommendations
