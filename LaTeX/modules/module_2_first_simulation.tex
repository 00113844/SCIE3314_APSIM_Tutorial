...
\begin{tikzpicture}[
  box/.style={rectangle, rounded corners=4pt, draw=UWABlue,
    fill=UWABlue!10, minimum width=3.5cm, minimum height=1.1cm,
    align=center, font=\bfseries},
  soilbox/.style={rectangle, rounded corners=6pt, draw=UWABlue,
    fill=UWABlue!18, minimum width=10cm, minimum height=2.2cm,
    align=center, font=\Large\bfseries\color{UWABlue}},
  lossbox/.style={rectangle, rounded corners=4pt, draw=WarningRed!70,
    fill=WarningRed!8, minimum width=3.5cm, minimum height=1.1cm,
    align=center, font=\bfseries\color{WarningRed!80!black}},
  arr/.style={-Stealth, very thick, UWAGrey}
]
  \node[box]     (rain)     at (0, 4.2)   {Rainfall ($P$)};
  \node[lossbox] (runoff)   at (6, 4.2)   {Runoff ($R_{\text{off}}$)};
  \node[soilbox] (soil)     at (0, 1.5)   {Soil Profile\\ESW (mm)\\between Wheat LL and DUL};
  \node[lossbox] (drainage) at (6, 1.5)   {Drainage ($D$)};
  \node[lossbox] (evap)     at (-5.2, 0)  {Soil Evap\\($E_s$)};
  \node[lossbox] (transp)   at (0, -1.0)  {Crop Transp\\($T_c$)};

  \draw[arr, UWABlue!70] (rain.south) -- node[left, font=\small\bfseries]{infiltration} (soil.north -| rain);
  \draw[arr, dashed, UWAGold, very thick] (rain.east) -- node[above, font=\small\bfseries\color{UWAGold!80!black}]{CN, bare soil} (runoff.west);
  \draw[arr, WarningRed!70] (soil.east) -- node[below, font=\small\color{WarningRed!70!black}]{SWCON} (drainage.west);
  \draw[arr, WarningRed!70] (soil.west) -- (evap.east);
  \draw[arr, WarningRed!70] (soil.south) -- (transp.north);
\end{tikzpicture}
...
\begin{tipbox}
\textbf{Note:} Additional reference material on ESW (Extractable Soil Water), Water Balance, Soil profiles, and Manager logic is available in the APSIM Next Gen documentation. For sowing rules and advanced water balance concepts, refer to Appendices~A and B of this tutorial.
\end{tipbox}
